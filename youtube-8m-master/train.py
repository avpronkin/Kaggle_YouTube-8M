...
if __name__ == "__main__":
  ...
  flags.DEFINE_bool(
      ...
      " new model instance.")
  flags.DEFINE_bool(
      "initialize_from_checkpoints", False,
      "If set, this will not resume from a checkpoint and will instead create a"
      " new model instance.")
  flags.DEFINE_string("checkpoints_file", "/tmp/yt8m_model/",
                      "The name of checkpoint to initialize varibles")
  ...
  flags.DEFINE_string("optimizer", "MomentumOptimizer",
                      "What optimizer class to use. MomentumOptimizer")
...
def build_graph(reader,
  ...
  optimizer = optimizer_class(learning_rate, momentum=0.9, use_nesterov = True)
...
#                           task_as_string(self.task))
  ...
  def run(self,start_new_model=False,
  	initialize_from_checkpoints=FLAGS.initialize_from_checkpoints,
  	checkpoints_file=FLAGS.checkpoints_file):
    ...
    with tf.Graph().as_default() as graph:
      ...
      with tf.device(device_fn):
        ...
        if initialize_from_checkpoints:
          previous_variables = [var_name for var_name, _ in tf.contrib.framework.list_variables(checkpoints_file)]
          restore_map = {variable.op.name:variable for variable in tf.global_variables()
                        if variable.op.name in previous_variables}
          tf.contrib.framework.init_from_checkpoint(checkpoints_file, restore_map)
